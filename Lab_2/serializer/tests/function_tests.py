...
from serializer.functions import function_serializer, source_code_serializer


CONST = 42
ANOTHERCONST = 12345678790**69


def save_function(f):
    function_serializer.function_dump(f, CUR_PATH + SOURCE_NAME)
    source_code_serializer.function_dump(f, CUR_PATH + CODE_NAME)


def load_function():
    fs = function_serializer.function_load(CUR_PATH + SOURCE_NAME)
    fc = source_code_serializer.function_load(CUR_PATH + CODE_NAME)
    return fs, fc
...
def test_f_consts():
    save_function(f_consts)
    x = 13
    for f in load_function():
        assert f(x) == f_consts_answer(x)


def test_f_recursion():
    save_function(f_recursion)
    x = 42
    for f in load_function():
